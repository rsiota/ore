...
- Relationships: `g r` docks a right-pane explorer (parents/children/files, or blame-line neighbourhood). `l` lazily expands a commit inline; `h` collapses; `Enter` jumps to a commit or file history. Soft DAG hops stay on the grid: `p` first parent, `c` first child, `u` merge-base with the view tip (`g p` / `g c` / `g u` aliases). `c` inverts the loaded log first, then `hash..tip`, then a session-cached `rev-list --children --all` (`Repo.ResetGraph` on refresh). Commit and blame-line views include an **Often with** section of co-changed file hot spots (labelled as a sample); `Enter` on a hot spot opens intersecting commits (`:couple`). An **Ownership** section lists top authors for the same paths (labelled `last N`); `Enter` opens that author's commits (`:authors`). File rows and line Previous/history show rename edges when known.
- Commit log window: default 500 (`git.DefaultLogLimit`). Title/status show `500/N` when capped; `:more` / `+` / `j` at the last row extend the window. Pickaxe / couple / authors / history / evolve titles append `N+` when they hit their cap.
- Detail / blame yank: `Tab` focuses a readonly VIEW browser (creel-inspired chords, not the textarea engine): `hjkl`/`wbe`/`0$`/`gg`/`G`, `f/t`, pane-local `/`, visual `v`/`V`, `y`/`Y`/`yy`/`yw`/`y$` to the system clipboard; reverse block cursor. On blame, `Tab` enters the code column first, then detail; elsewhere `Tab` enters detail (or the hunk strip when `H` is on). `[`/`]` (and `{`/`}`) jump hunk headers in detail yank; with the hunk strip open they also hop from the main grid. `esc` peels visual then returns focus to the main grid. On the main grid without hunk mode, `[`/`]` still adjust zen diff context.
- Hunk list: `H` / `:hunks` docks a bottom strip of `@@` headers for the current detail patch; focus with `Tab`, navigate with `j/k`, `Enter` jumps the detail viewport.
