...
- Navigation stack: **commits → files → history → blame → evolve** (`Enter` / `b` forward; `F` opens the line evolution stack from blame; `l` still opens on files/history/blame; `esc` back). Pickaxe (`:pickaxe` / `:S` / `:G`) opens a content-history results grid and washes matching spans in detail/blame until `:nohl` or leaving pickaxe. History uses `git log --follow`; blame uses `git blame --porcelain`; evolution walks porcelain `previous`; detail uses `Show` / `ShowPath`. One-hop follow remains `f` / `g f`.
...
- Relationships: `g r` docks a right-pane explorer (parents/children/files, or blame-line neighbourhood). `l` lazily expands a commit inline; `h` collapses; `Enter` jumps to a commit or file history. Commit and blame-line views include an **Often with** section of co-changed file hot spots.
