...
- Detail yank: `Tab` focuses the detail pane as a readonly VIEW browser (creel-inspired chords, not the textarea engine): `hjkl`/`wbe`/`0$`/`gg`/`G`, `f/t`, pane-local `/`, visual `v`/`V`, `y`/`Y`/`yy`/`yw`/`y$` to the system clipboard; reverse block cursor; `[`/`]` (and `{`/`}`) jump hunk headers; `esc` peels visual then returns focus to the main grid. On the main grid, `[`/`]` still adjust zen diff context.
