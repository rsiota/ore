...
- Navigation stack: **commits → files → history → blame → evolve** (`Enter` / `b` forward; `F` opens the line evolution stack from blame; `l` still opens on files/history/blame; `esc` back). Pickaxe (`:pickaxe` / `:S` / `:G`) opens a content-history results grid. History uses `git log --follow`; blame uses `git blame --porcelain`; evolution walks porcelain `previous`; detail uses `Show` / `ShowPath`. One-hop follow remains `f` / `g f`.
